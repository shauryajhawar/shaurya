...
function setup() {
  ...
  ground=createSprite(200,120,width,height);
  ...
  gameOver = createSprite(width - 170,height-300,10,10); 
  gameOver.addImage("GameOverT",gameOverI);
  gameOver.scale=0.5;
  gameOver.visible=false;

  restart = createSprite(width - 170,height - 170,10,10);
...
function draw() {
 ...
 // monkey.collide(b1);
  if(gameState===play){
   ...
   if( touches.length > 0 && monkey.y>=330 ){
   monkey.velocityY = -12;
      touches = [];
     }
  ...
  fill("white");
  textSize(18);
  text("Your score:-"+score,width-295,20);
}
...
function spawnBanana(){
  if (frameCount%120===0) {
    ...
    banana.x=610;
    banana.y=random(height - 250,height - 300 );
...
if (frameCount%140===0) {
    uphill = createSprite(610, height - 200,10,10);
    uphill.x=610;
